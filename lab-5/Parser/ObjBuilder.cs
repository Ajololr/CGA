...
namespace ObjParser
{
    ...
    {
        private IList<Vector4> _vertices = new List<Vector4>();
        ...
        internal void AddPosition(in ReadOnlySpan<string> position)
        {
            ...
            _vertices.Add(ParseVector4(position));
        }
        ...
        public void AddFace(in ReadOnlySpan<string> face)
        {
            ...
            {
                var values = face[i].Split('/')
                    .Select(x => {
                        var xParsed = int.Parse(x);
                         return  xParsed > 0 ? xParsed - 1 : _vertices.Count + xParsed;
                        
                    }) // Subtract 1 since .obj file array index starts at 1
                    .ToArray();
        ...
        private static Vector4 ParseVector4(in ReadOnlySpan<string> values) => new Vector4(ParseFloat(values[0]), ParseFloat(values[1]), ParseFloat(values[2]), 1);
