...
namespace ObjParser
{
    internal class MaterialBuilder
    {
        ...
        public void SetNormalMap(in ReadOnlySpan<string> values)
        {
            if (values.Length != 1)
            {
                throw new FormatException("Normal Map must be in format map_norm <path>");
            }
            _currentMaterial.NormalMap = values[0];
        }
